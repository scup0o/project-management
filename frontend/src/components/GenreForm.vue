        ...
        <div class="modal-wrapper">
            <div class="modal-container">
                <div class="modal-header">
                    <label class="" style="margin: auto; font-size: 20px;" v-if="edit===false">Thêm thể loại</label>
                    <label class="" style="margin: auto; font-size: 20px;" v-else>Chỉnh sửa thể loại</label>

                </div>
                    <Form @submit="createGenre" :validation-schema="FormSchema">
                        <div class="modal-body">
                        <div class="row">
                            <div class="form-group">
                                <label for="name">Tên thể loại: </label>
                                <Field name="name" type="text" class="form-control" v-model="genreNew.name">
                                </Field>
                        ...
                        <div style="padding-top:20px">
                            ...
                            <form encType="multipart/form-data" @submit="this.$emit('submit')" action="/api/img" method="post">
                                ...
                                <div class="row">
                                <div v-if="genreNew.image && haveData===true">
                                    <img style="display: inline-flex;" class="preview img-thumbnail" :src="`../../src/assets/img/${genreNew.image}`"/>
                                    {{ genreNew.image }}
                                </div>
    ...
    methods: {
        ...
        async createGenre(data) {
            try {
                ...
                if (this.edit === false){
                    const check = await GenreService.create(data);
                    if (check === false){
                    ...
                    const check = await GenreService.update(this.genreNew._id, data);
                    if (check === false){
                        this.nameMessage = 'Tên thể loại đã tồn tại'
                    }
                    else{
